...
import { forgotPassword } from "@/app/features/trainer/auth/authService";
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          router.push(`/trainer-verify-code?email=${encryptedEmail}`);
        }, 2000);
            ...
            <div className="log_wraps">
              ...
              <div className="log__heads">
                <h4 className="logs_title">
                  {" "}
                  {trainer?.trainer_forgot_password_title || "Reset Password"}
                </h4>
                {<p>{trainer?.trainer_forgot_password_description} </p> || (
                  <p>
              ...
              <div className="back-login form-group text-center">
                <Link href="/trainer-login">Back To login</Link>
              </div>
            </div>
          </div>
        </div>
